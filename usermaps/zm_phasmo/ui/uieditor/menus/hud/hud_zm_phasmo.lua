...
require("ui.uieditor.widgets.hud._pmzm.pmzm_thermometer_upgraded")
...
--	HudRef.DevWins = DevText
	...
	---------------------------------------------------------------------------
	-- Custom PMZM functionality
	---------------------------------------------------------------------------

	local function ViewmodelWpnNameCallback(ModelRef)
		if IsCurrentViewmodelWeaponName(InstanceRef, "pmzm_thermometer_upgraded")
				or IsCurrentViewmodelWeaponName(InstanceRef, "pmzm_thermometer_upgraded_alt") then
			if HudRef.PmzmThermometerUpgraded == nil then
				HudRef.PmzmThermometerUpgraded = CoD.PmzmThermometerUpgraded.new(HudRef, InstanceRef)
				HudRef:addElement(HudRef.PmzmThermometerUpgraded)
				HudRef.PmzmThermometerUpgraded:dispatchEventToChildren({
					name = "update_state",
			...
			if HudRef.PmzmThermometerUpgraded ~= nil then
				HudRef.PmzmThermometerUpgraded:close()
				HudRef.PmzmThermometerUpgraded = nil
			end
		end
	end
	HudRef:subscribeToModel(
		...
		ViewmodelWpnNameCallback
	)

	---------------------------------------------------------------------------
	-- End custom PMZM functionality
	---------------------------------------------------------------------------
